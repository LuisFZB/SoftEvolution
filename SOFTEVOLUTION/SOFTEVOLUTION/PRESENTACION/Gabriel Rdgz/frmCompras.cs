...
namespace SOFTEVOLUTION
{
    ...
    {
        public frmCompras(ToolStripLabel Usuario, ToolStripMenuItem Guardar)
        {
            InitializeComponent();
            usuario = Usuario;
            guardar = Guardar;
                    ...
                    for (int i = 0; i < tamaño; i++)
                    {
                        if (lstProductos.Items[i].SubItems[0].Text != "")
                        {
                            try
                            {
                                ...
                                consulta.ActualizarBodega(lstProductos.Items[i].SubItems[0].Text, Int32.Parse(lstProductos.Items[i].SubItems[2].Text));
